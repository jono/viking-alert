...
desc "Compile mls.js"
task :compile do
  FileUtils.rm_f('./mls.js')

  File.open('./viking.alert.js', "w") do |file|
    file << environment['viking-alert.js'].to_s
    file << environment['templates/alert-view.js'].to_s
    # <<-DOC
    #     (function () {
    #         'use strict';
    #         #{environment['mls.js'].to_s}
    #     }());
    # DOC
  end
